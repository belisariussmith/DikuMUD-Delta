/* ************************************************************************
*  file: board.c, Implementation of virtual board system. Part of DIKUMUD *
*  Usage : Virtual board commands.                                        *
...
//
// Virtual board related functions
//

int board(struct char_data *ch, int cmd, char *arg)
{
    static int has_loaded = FALSE;
...
void board_write_msg(struct char_data *ch, char *arg)
{
    ...
    // Is this clumsy?  - four strcat() in a row I mean.. (yes, and its just three ;)
    strcat(head[msg_num], " (");
...
void board_load_board()
{
    ...
    if (!fread(&msg_num, sizeof(int), 1, the_file))
    {
    ...
    for (ind = 0; ind < msg_num; ind++)
    {
        if (!fread(&len, sizeof(int), 1, the_file))
        {
            error_log("Reading board msg failed..\n\r");
            board_reset_board();
            fclose(the_file);
            return;
        }
        ...
        if (!fread(head[ind], sizeof(char), len, the_file))
        {
            error_log("Reading board msg failed..\n\r");
            board_reset_board();
            fclose(the_file);
        ...
        if (!fread(msgs[ind], sizeof(char), len, the_file))
        {
            error_log("Reading board msg failed..\n\r");
            board_reset_board();
            fclose(the_file);
...
int board_display_msg(struct char_data *ch, char *arg)
{
    ...
    //  sprintf(buf, "$n reads message %d titled : %s.", msg, head[msg - 1]);
