...
struct free_list_struct *new_free(long offset)
{
...
struct mail_offset_struct *new_offset()
{
    ...
    p->mail_header_offset = 1L;                            // 1L can hardly be a regular blocksize?
    p->my_next_mail = NULL;

    return p;
}

struct mail_index_struct *new_index(struct mail_offset_struct *first)
{
...
struct mail_index_struct *get_index_pos(char name[])
{
...
void add_free(long offset, struct free_list_struct *p)
{
    ...
    {
        fprintf(stderr, "Error. Trying to add free offset %lu twice. Ignored.\n", offset);
        free(new);
        return;
    }
...
int add_free_list(long offset)
{
    ...
    if (offset == 0)
    {
        ...
        fread_result = fread(&offset, LONGSIZE, 1, mailfp);
        blocks++;
    }
...
void add_mail(char to[], char from[], long offset)
{
    extern struct mail_index_struct *mail_index;
    ...
    while (strncmp(mi->to, to, NAMESIZE))
    {
        // Check for name in index list
        if (mi->next == NULL)
        {
            // else create new index-element
...
//            1 - if the mail_offset_struct element was removed without remarks
//            2 - if the mail_index_struct element was also removed because
//                there were no more mails to the player.
/////////////////////////////////////////////////////////////////////////////
...
//  a NULL value.
/////////////////////////////////////////////////////////////////////////////
int remove_offset(struct mail_index_struct *cur_ind, struct mail_offset_struct *cur_off)
{
    extern struct mail_index_struct *mail_index;
    extern struct free_list_struct *free_list;
...
char *read_delete(char recipient[])
{
    ...
    fread_result = fread(&hbl, BLOCKSIZE, 1, mailfp);
    ...
    if (hbl.offset || (hbl.offset == 0))
    {
        ...
        do
        {
            ...
            //dbl.offset = NULL;
            dbl.offset = 0;
            fread_result = fread(&dbl, BLOCKSIZE, 1, mailfp);

            if ((text = (char *)realloc(text, (gobbledigook + DATABLOCKSIZE))) == NULL)
            {
                fprintf(stderr, "Couldn't realloc %ld more bytes for mail.\n", DATABLOCKSIZE);
                free(text);
                return NULL;
            }
...
int delete_old(struct free_list_struct *base)
{
...
//  in maildef.h.
//  If scan_file returns 0/FALSE, disable the mailsystem.
...
int scan_file(char *file)
{
    ...
    if ((mail_index = (struct mail_index_struct *)mymalloc(INDEXSIZE)) == NULL)
    {
            return 0;
    }
    ...
    fread_result = fread(tmp_hbl, BLOCKSIZE, 1, mailfp);
    ...
    while (1)
    {
        ...
        else if (byte == 1)
        {
            ...
            //fread_result = fread((tmp_hbl + 1), BLOCKSIZE - 1, 1, mailfp);
            fread_result = fread((tmp_hbl), BLOCKSIZE - 1, 1, mailfp);
...
#endif
    fprintf(stderr, "***************************** MAILSYS *****************************\n");
    fprintf(stderr, "Mailfile read : %s, size was %lu bytes, ", MAILFILE, end_of_file);
    fprintf(stderr, "blocksize %d\nTotal # of unread mails %lu\n", BLOCKSIZE, no_of_mails);
...
void writeblock(char *buf, long nextpos)
{
    extern FILE *mailfp;
    ...
    {
        fprintf(stderr, "Fwrite at fpos %lu failed.\n", ftell(mailfp));
        return;
    }
}
...
void store_mail(char *to, char *from, char *buf)
{
    FILE *mailfp;
    extern long end_of_file;
    ...
    strncpy(shovel + BLOCKSIZE - (LONGSIZE + HEADBLOCKSIZE), buf, HEADBLOCKSIZE);

    printf("2..\n");

    if (end_of_file == 0)
    {
        // eof stts av scan_file
    ...
    if (buflen <= HEADBLOCKSIZE)
    {
        ...
        writeblock(shovel, 0); // If it's just a header block
