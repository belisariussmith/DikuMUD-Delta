    ...
    else
    {
        ...
        switch(keyword_no)
        {
            ...
            case 6:
            {
                ...
                {
                    // Item carried
                    flags = (FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP);

                    bits  = generic_find(arg2, flags, ch, &tmp_char, &tmp_object);
                    ...
                    if (bits)
                    {
                        ...
                        if (GET_ITEM_TYPE(tmp_object)== ITEM_DRINKCON)
                        {
                            ...
                            else
                            {
                                ...
                                sprintf(buffer, "It's %sfull of a %s liquid.\n\r", fullness[temp], color_liquid[tmp_object->obj_flags.value[2]]);
                                send_to_char(buffer, ch);
                            }
                        }
            ...
            case 7 :
            {
                if (*arg2)
                {
                    flags = (FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP | FIND_CHAR_ROOM);
                    bits  = generic_find(arg2, flags, ch, &tmp_char, &found_object);
...
void do_examine(struct char_data *ch, char *argument, int cmd)
{
    ...
    bits  = generic_find(name, flags, ch, &tmp_char, &tmp_object);
