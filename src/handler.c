...
void affect_remove( struct char_data *ch, struct affected_type *af )
{
    ...
    if (!ch->affected)
    {
        mudlog("ERROR: attempt to remove affect from (un) affected ch in affect_remove()");
        return;
    }
    ...
    else
    {
        ...
        if (hjp->next != af)
        {
            mudlog("FATAL : Could not locate affected_type in ch->affected. (handler.c, affect_remove)");
            exit(1);
        }
...
void char_from_room(struct char_data *ch)
{
    ...
    else
    {
        ...
        for (i = world[ch->in_room].people; i->next_in_room != ch; i = i->next_in_room) ;
...
int apply_ac(struct char_data *ch, int eq_pos)
{
    ...
    {
        mudlog("ERROR: attempt to retrieve AC of non-existent equipment in apply_ac()");
        return 0;
    }
    ...
    {
        case WEAR_BODY:
            // Body armor is weighted 30%
            return (3*ch->equipment[eq_pos]->obj_flags.value[0]);

        case WEAR_HEAD:
            // Head armor is weighted 20%
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
    ...
    {
        mudlog("ERROR: Attempt to equip a ch(aracter) to an invalid pos(ition) in equip_char()");
        return;
    }

    if (ch->equipment[pos])
    {
        mudlog("ERROR: Attempt to double-equip a ch(aracter) in equip_char()");
        return;
    }

    if (obj->carried_by)
    {
        mudlog("EQUIP: Obj is carried_by when equipping in equip_char()");
        return;
    }

    if (obj->in_room != NOWHERE)
    {
        mudlog("EQUIP: Obj is in_room when equipping in equip_char()");
        return;
    }
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
    ...
    if ( (pos < 0) || (pos >= MAX_WEAR) )
    {
        mudlog("ERROR: invalid pos(ition)");
        return NULL;
    }

    if (!ch->equipment[pos])
    {
        mudlog("ERROR: no equipment for ch(aracter) at designated position in unequip_char()");
        return NULL;
    }
...
void extract_char(struct char_data *ch)
{
    ...
    {
        // leaves nothing !
        mudlog("NOWHERE, extracting char.");
        exit(1);
    }
    ...
    else
    {
        ...
        {
            mudlog("Trying to remove ?? from character_list. (handler.c, extract_char)");
            abort();
        }
    }
