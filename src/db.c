...
void boot_db(void)
{
    ...
    mudlog("Reading newsfile, credits, help-page, info and motd.");
    ...
    mudlog("Generating index tables for mobile and object files.");
    ...
    mudlog("Updating characters with saved items:");
    update_obj_file();
...
void reset_time(void)
{
    ...
    // 650336715 = GMT: Saturday, August 11, 1990 1:05:15 AM
    ...
    fscanf_result = fscanf(f1, "#\n");

    fscanf_result = fscanf(f1, "%D\n", &last_time);
...
void build_player_index(void)
{
    ...
    for (; !feof(fl);)
    {
        fread_result = fread(&dummy, sizeof(struct char_file_u), 1, fl);
...
void boot_world(void)
{
    ...
    {
        perror("fopen");
        mudlog("boot_world: could not open world file.");
        exit(0);
    }

    do
    {
        fscanf_result = fscanf(fl, " #%d\n", &virtual_nr);
...
void boot_zones(void)
{
    ...
    for (;;)
    {
        ...
        {
            if (!(zone_table = (struct zone_data *) realloc(zone_table, (zon + 1) * sizeof(struct zone_data))))
            {
        ...
        fscanf_result = fscanf(fl, " %d ", &zone_table[zon].top);
        fscanf_result = fscanf(fl, " %d ", &zone_table[zon].lifespan);
        fscanf_result = fscanf(fl, " %d ", &zone_table[zon].reset_mode);
        ...
        for (expand = TRUE;;)
        {
            if (expand)
            {
                ...
                {
                    if (!(zone_table[zon].cmd = (struct reset_com *) realloc(zone_table[zon].cmd, (cmd_no + 1) * sizeof(struct reset_com))))
                    {
            ...
            fscanf_result = fscanf(fl, " "); // skip blanks
            fscanf_result = fscanf(fl, "%c", &zone_table[zon].cmd[cmd_no].command);
            ...
            if (zone_table[zon].cmd[cmd_no].command == '*')
            {
                ...
                {
                    // May want to do something in case of NULL
                }
                continue;
            }

            fscanf_result = fscanf(fl, " %d %d %d",
                &tmp,
            ...
            if (zone_table[zon].cmd[cmd_no].command == 'M' ||
                ...
                fscanf_result = fscanf(fl, " %d", &zone_table[zon].cmd[cmd_no].arg3);
...
struct char_data *read_mobile(int nr, int type)
{
    ...
    fscanf_result = fscanf(mob_f, "%ld ", &tmp);
    ...
    if (letter == 'S')
    {
        ...
        fscanf_result = fscanf(mob_f, " %ldd%ld+%ld ", &tmp, &tmp2, &tmp3);
        mob->points.max_hit = dice(tmp, tmp2)+tmp3;
        mob->points.hit     = mob->points.max_hit;

        fscanf_result = fscanf(mob_f, " %ldd%ld+%ld \n", &tmp, &tmp2, &tmp3);
    ...
    else
    {
        ...
        fscanf_result = fscanf(mob_f, " %ld ", &tmp);
        fscanf_result = fscanf(mob_f, " %ld ", &tmp2);
...
struct obj_data *read_object(int nr, int type)
{
    ...
    fscanf_result = fscanf(obj_f, " %d ", &tmp);
    ...
    while (fscanf_result = fscanf(obj_f, " %s \n", chk), *chk == 'E')
    {
        CREATE(new_descr, struct extra_descr_data, 1);
...
/*
   ...
   "Load a char, TRUE if loaded, FALSE if nothing"

   However, this function actually returns the number of the player
   in the MUD's player list, its "ID" number in a sense. The number
   can be 0 (the first in the list).

   So, this function actually returns the player ID or (-1) which
   is interpreted as "No player found with that name" - belisarius
*/
...
int load_char(char *name, struct char_file_u *char_element)
{
    ...
    if ((player_i = find_name(name)) >= 0)
    {
        ...
        fread_result = fread(char_element, sizeof(struct char_file_u), 1, fl);
        fclose(fl);
...
int file_to_string(char *name, char *buf)
{
    ...
    *buf = '\0';
    ...
    do
    {
        if (fgets(tmp, 99, fl) == NULL)
        {
            // May be EOF, can add other check
        }
