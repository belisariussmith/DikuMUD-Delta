...
int guild(struct char_data *ch, int cmd, char *arg)
{
    ...
    switch (GET_CLASS(ch))
    {
        ...
        case CLASS_WARRIOR:
        {
            ...
            percent = ch->skills[number+SKILL_KICK].learned + MIN(12, int_app[GET_INT(ch)].learn);
        ...
        default:
            mudlog("ERROR: invalid class in guild spec_proc");
            return (FALSE);
    }
...
int snake(struct char_data *ch, int cmd, char *arg)
{
        ...
        (number(0, 32-GET_LEVEL(ch)) == 0) )
    {
        ...
        cast_poison( GET_LEVEL(ch), ch, "", SPELL_TYPE_SPELL, ch->specials.fighting, 0);
...
int chalice(struct char_data *ch, int cmd, char *arg)
{
    ...
    switch(cmd)
    {
        case 10:
            // get
            if (!(chalice = get_obj_in_list_num(chl, world[ch->in_room].contents)) && CAN_SEE_OBJ(ch, chalice))
            {
                if (!(chalice = get_obj_in_list_num(achl, world[ch->in_room].contents)) && CAN_SEE_OBJ(ch, chalice))
                {
                    return(FALSE);
                }
            }
