...
void boot_social_messages(void)
{
    ...
    for (;;)
    {
        ...
        {
            if (!(soc_mess_list = (struct social_messg *) realloc(soc_mess_list, sizeof (struct social_messg) * (++list_top + 1))))
            {
...
void boot_pose_messages(void)
{
    ...
    {
        if (!fscanf(fl, " %d ", &pose_messages[counter].level))
        {
            break;
        }
...
void do_pose(struct char_data *ch, char *argument, int cmd)
{
    ...
    for ( counter = 0;
         (pose_messages[counter].level <= GET_LEVEL(ch)) && (pose_messages[counter].level >= 0);
          counter++ ) ;
