...
void shopping_buy( char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if (GET_GOLD(ch) < (int) (temp1->obj_flags.cost * shop_index[shop_nr].profit_buy) && (GET_LEVEL(ch) < 22) )
    {
    ...
    if (keeper->carrying)
    {
        for (temp1 = keeper->carrying; temp1; temp1 = temp1->next_content)
        {
            if ( (CAN_SEE_OBJ(ch, temp1)) && (temp1->obj_flags.cost > 0))
            {
                ...
                {
                    sprintf(buf2,"%s for %d gold coins.\n\r", (temp1->short_description), (int)(temp1->obj_flags.cost*shop_index[shop_nr].profit_buy));
                }
                else
                {
...
void boot_the_shops()
{
    ...
    for(;;)
    {
        ...
        if (*buf == '#')
        {
            ...
            {
                if (!fscanf(shop_f, "%d \n", &temp)) { perror("Error in shop file : for (count < MAX_PROD)"); exit(0); }
            ...
            if (!fscanf(shop_f, "%f \n", &shop_index[number_of_shops].profit_buy))  { perror("Error in shop file : for (;;) profit_buy");  exit(0); }
            if (!fscanf(shop_f, "%f \n", &shop_index[number_of_shops].profit_sell)) { perror("Error in shop file : for (;;) profit_sell"); exit(0); }

            for(count = 0; count < MAX_TRADE; count++)
            {
                if (!fscanf(shop_f, "%d \n", &temp)) { perror("Error in shop file : for (count < MAX_TRADE)"); exit(0); }
            ...
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].temper1)) { perror("Error in shop file : for (;;) 2"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].temper2)) { perror("Error in shop file : for (;;) 3"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].keeper))  { perror("Error in shop file : for (;;) 4"); exit(0); }

            shop_index[number_of_shops].keeper = real_mobile(shop_index[number_of_shops].keeper);

            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].with_who)) { perror("Error in shop file : for (;;) 5"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].in_room))  { perror("Error in shop file : for (;;) 6"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].open1))    { perror("Error in shop file : for (;;) 7"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].close1))   { perror("Error in shop file : for (;;) 8"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].open2))    { perror("Error in shop file : for (;;) 9"); exit(0); }
            if (!fscanf(shop_f, "%d \n", &shop_index[number_of_shops].close2))   { perror("Error in shop file : for (;;) A"); exit(0); }
