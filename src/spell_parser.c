...
//
// Parsing procedures and basic routines for Spells
//
...
void clone_char(struct char_data *ch)
{
    ...
    // Clear EVERYTHING! (ASSUMES CORRECT)
    clear_char(clone);
...
void clone_obj(struct obj_data *obj)
{
    ...
    *clone = *obj;
    ...
    // VIRKER IKKE ENDNU (DOESNT WORK YET)
}
...
void stop_follower(struct char_data *ch)
{
    ...
    if (!ch->master)
    {
        mudlog("ERROR: No master for ch(aracter) passed to stop_follower()");
        return;
    }
...
void add_follower(struct char_data *ch, struct char_data *leader)
{
    ...
    if (ch->master)
    {
        mudlog("ERROR: master already set for ch(aracter) passed to add_follower()");
        return;
    }
