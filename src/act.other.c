...
void do_steal(struct char_data *ch, char *argument, int cmd)
{
    ...
    else
    {
        ...
        else
        {
            ...
            gold = MIN(1782, gold);      // Maximum of 1,782 coins allowed to be stolen
...
void do_pick(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (percent > (ch->skills[SKILL_PICK_LOCK].learned + dex_app_skill[GET_DEX(ch)].p_locks) )
    {
...
void do_quaff(struct char_data *ch, char *argument, int cmd)
{
    ...
    for (i = 1; i < 4; i++)
    {
        ...
        {
          ((*spell_info[temp->obj_flags.value[i]].spell_pointer)((byte) temp->obj_flags.value[0], ch, "", SPELL_TYPE_POTION, ch, 0));
        }
    }
...
void do_recite(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (*argument)
    {
        int bitmask = (FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP | FIND_CHAR_ROOM);

        bits = generic_find(argument, bitmask, ch, &victim, &obj);
    ...
    for (i = 1; i < 4; i++)
    {
        ...
        {
            ((*spell_info[scroll->obj_flags.value[i]].spell_pointer)((byte) scroll->obj_flags.value[0], ch, "", SPELL_TYPE_SCROLL, victim, obj));
        }
    }
...
void do_use(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (stick->obj_flags.type_flag == ITEM_STAFF)
    {
        ...
        if (stick->obj_flags.value[2] > 0)
        {
            ...
            ((*spell_info[stick->obj_flags.value[3]].spell_pointer)((byte) stick->obj_flags.value[0], ch, "", SPELL_TYPE_STAFF, 0, 0));
        }
        else
        {
    ...
    {
        int bitmask = (FIND_CHAR_ROOM | FIND_OBJ_INV | FIND_OBJ_ROOM | FIND_OBJ_EQUIP);

        bits = generic_find(argument, bitmask, ch, &tmp_char, &tmp_object);
        ...
        if (bits)
        {
            ...
            if (stick->obj_flags.value[2] > 0)
            {
                // Is there any charges left?
                stick->obj_flags.value[2]--;
                ((*spell_info[stick->obj_flags.value[3]].spell_pointer)((byte) stick->obj_flags.value[0], ch, "", SPELL_TYPE_WAND, tmp_char, tmp_object));
            }
            else
            {
