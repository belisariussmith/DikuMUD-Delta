...
//
// Character loading/saving & Inn-related procedures
//
...
bool recep_offer(struct char_data *ch, struct char_data *receptionist, struct obj_cost *cost)
{
    ...
    {
        sprintf(buf,"$n tells you 'Sorry, but I can't store any more than %d items.", MAX_OBJ_SAVE);
    ...
    if (cost->total_cost > GET_GOLD(ch))
    {
        if (GET_LEVEL(ch) < 21)
        {
            act("$n tells you 'Which I can see you can't afford'", FALSE, receptionist, 0, ch, TO_VICT);
        }
        else
        {
            act("$n tells you 'Well, since you're a God, I guess it's okay'", FALSE, receptionist, 0, ch, TO_VICT);
            cost->total_cost = 0;
        }
    }
...
void load_char_objs(struct char_data *ch)
{
    ...
    if (found)
    {
        ...
        timegold = (unsigned) (((float) st.total_cost*(float) (time(0) - st.last_update)) / (float) (SECS_PER_REAL_DAY));
        ...
        if (fwrite(&st, sizeof(struct obj_file_u), 1, fl) < 1)
        {
            mudlog("Error updating name to be set as unused.");
            exit(1);
        }
    ...
    {
        mudlog("Char has no data in file!");
    }
...
void put_obj_in_store(struct obj_data *obj, struct obj_file_u *st)
{
    ...
    if (!found)
    {
        mudlog("No empty space to store object. (put_obj_in_store, reception.c)");
        exit(1);
    }
}

// Destroy inventory after transferring it to "store inventory"
void obj_to_store(struct obj_data *obj, struct obj_file_u *st, struct char_data * ch)
{
    static char buf[240];
        ...
        if ((!feof(fl)) && (no_read > 0) && st.owner[0])
        {
            ...
            if (days_passed > 0)
            {
                if ((st.total_cost*days_passed) > st.gold_left)
                {
                    ...
                    if (!fread(&ch_st, sizeof(struct char_file_u), 1, char_file))
                    {
                        mudlog("ERROR: failure in reading object file.");
                    }
...
int receptionist(struct char_data *ch, int cmd, char *arg)
{
    ...
    for (temp_char = world[ch->in_room].people; (temp_char) && (!recep); temp_char = temp_char->next_in_room)
    {
        if (IS_MOB(temp_char))
        {
