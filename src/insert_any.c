...
void merge(FILE *fl1, FILE *fl2)
{
    ...
    if (!fgets(buf1, MAX_LINE, fl1))
    {
        printf("Error reading file 1 during merge.\n");
        exit(1);
    }
    antal = sscanf(buf1, " #%d ", &num1);
    assume(antal, 1, 0, "No #xxxx found next (old)");

    if (!fgets(buf2, MAX_LINE, fl2))
    {
        printf("Error reading file 2 during merge.\n");
        exit(1);
    }
