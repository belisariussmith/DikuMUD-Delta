...
//
// Object interaction related procedures
//
...
void do_pour(struct char_data *ch, char *argument, int cmd)
{
    ...
    sprintf(buf, "You pour the %s into the %s.", drinks[from_obj->obj_flags.value[2]], arg2);
    send_to_char(buf,ch);
    ...
    // from_obj is empty
    if ( (from_obj->obj_flags.value[1] = 0) )    // Assignment instead of comparison?
    {
        from_obj->obj_flags.value[1] = 0;
        from_obj->obj_flags.value[2] = 0;
        from_obj->obj_flags.value[3] = 0;
    ...
    if (*arg1)
    {
        ...
        if (obj_object)
        {
            ...
            else
            {
                ...
                if (CAN_WEAR(obj_object, ITEM_WEAR_SHIELD)) keyword = EQUIP_SHIELD;
                if (CAN_WEAR(obj_object, ITEM_WEAR_FINGER)) keyword = EQUIP_FINGER;
                if (CAN_WEAR(obj_object, ITEM_WEAR_NECK))   keyword = EQUIP_NECK;
                if (CAN_WEAR(obj_object, ITEM_WEAR_WRIST))  keyword = EQUIP_WRIST;
                if (CAN_WEAR(obj_object, ITEM_WEAR_WAISTE)) keyword = EQUIP_WAIST;
                if (CAN_WEAR(obj_object, ITEM_WEAR_ARMS))   keyword = EQUIP_ARMS;
                if (CAN_WEAR(obj_object, ITEM_WEAR_HANDS))  keyword = EQUIP_HANDS;
                if (CAN_WEAR(obj_object, ITEM_WEAR_FEET))   keyword = EQUIP_FEET;
                if (CAN_WEAR(obj_object, ITEM_WEAR_LEGS))   keyword = EQUIP_LEGS;
                if (CAN_WEAR(obj_object, ITEM_WEAR_ABOUT))  keyword = EQUIP_ABOUT;
                if (CAN_WEAR(obj_object, ITEM_WEAR_HEAD))   keyword = EQUIP_HEAD;
                if (CAN_WEAR(obj_object, ITEM_WEAR_BODY))   keyword = EQUIP_BODY;
...
void do_wield(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (*arg1)
    {
        ...
        if (obj_object)
        {
            wear(ch, obj_object, EQUIP_WIELD);
        }
        else
        {
            sprintf(buffer, "You do not seem to have the '%s'.\n\r", arg1);
...
void do_grab(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (*arg1)
    {
        ...
        if (obj_object)
        {
            ...
            {
                wear(ch, obj_object, EQUIP_LIGHT);
            }
            else
            {
                wear(ch, obj_object, EQUIP_HOLD);
            }
        }
        else
        {
