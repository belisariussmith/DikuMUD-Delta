...
void load_messages(void)
{
    ...
    if (!fscanf(f1, " %s \n", chk))
    {
        mudlog("Failed to read combat messages.");
        exit(0);
    }
    ...
    while(*chk == 'M')
    {
        if (!fscanf(f1," %d\n", &type))
        {
            mudlog("Failure while reading combat messages. (type)");
            exit(0);
        }
        ...
        if (!fscanf(f1, " %s \n", chk))
        {
            mudlog("Failure while reading combat messages. (chk)");
            exit(0);
        }
    }
...
void set_fighting(struct char_data *ch, struct char_data *vict)
{
    ...
    {
        mudlog("ERROR: ch->specials.fighting is set entering set_fighting()");
        return;
    }
...
void stop_fighting(struct char_data *ch)
{
    ...
    if (!ch->specials.fighting)
    {
        mudlog("ERROR: ch->specials.fighting is not set entering stop_fighting()");
        return;
    }
...
void make_corpse(struct char_data *ch)
{
    ...
    sprintf(buf, "Corpse of %s is lying here.", (IS_NPC(ch) ? ch->player.short_descr : GET_NAME(ch)));
...
void damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
    ...
    {
        mudlog("ERROR: victim position is <= dead in damage()");
        return;
    }

    if ( (GET_LEVEL(victim) > 20) && (!IS_NPC(victim)) )
    {
    ...
    {
        // Max 18 damage when sanctuary
        dam = MIN(dam, 18);
    }

    // Damage should not exceed 100
    dam = MIN(dam, 100);
...
void perform_violence(void)
{
    ...
    for (ch = combat_list; ch; ch = combat_next_dude)
    {
        ...
        if (!ch->specials.fighting)
        {
            mudlog("ERROR: ch->specials.fighting not set entering perform_violence()");
            return;
        }
